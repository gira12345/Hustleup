...
export default function EditarEmpresaGestor() {
  const { id } = useParams();
  ...
  useEffect(() => {
    async function fetchEmpresa() {
      try {
        const res = await api.get(`/api/gestor/empresas/${id}`);
        const data = res.data;
        setEmpresa({
          nome: data.nome || "",
          email: data.email || "",
  ...
  const handleSubmit = async (e) => {
    ...
    setSucesso("");
    try {
      await api.put(`/api/gestor/empresas/${id}`, empresa);
      setSucesso("Empresa atualizada com sucesso!");
      setTimeout(() => navigate("/gestor/empresas"), 1500);
    } catch (err) {
    ...
    <Card className="shadow-sm border-0 rounded-4 p-4 max-w-2xl w-full mx-auto">
      <div className="flex items-center gap-2 mb-4">
        <button
          ...
          onClick={() => navigate('/gestor/empresas')}
        >
      ...
      <form onSubmit={handleSubmit} className="flex flex-col space-y-5">
        ...
        <div className="flex flex-col">
          ...
          <input
            ...
            onChange={handleChange}
            className="text-sm px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
            placeholder="Telefone ou outro contacto"
          />
        </div>
        <div className="flex flex-col">
        ...
        <button
          type="submit"
          className="bg-blue-400 hover:bg-blue-500 text-blue-50 px-3 py-1 rounded text-xs font-semibold border-0 transition-all mr-2 shadow-sm min-w-[90px]"
        >
          Guardar Alterações
        </button>
      </form>
