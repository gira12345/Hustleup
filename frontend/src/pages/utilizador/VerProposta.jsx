...
export default function VerPropostaUtilizador() {
  const { id } = useParams();
  ...
  const toggleFavorito = async () => {
    try {
      ...
      alert("Erro ao atualizar favorito. Tenta novamente.");
    }
  };
  ...
  if (error) {
    return (
      <div className="px-8 py-10 max-w-4xl mx-auto">
        ...
        <button
          type="button"
          onClick={() => navigate("/utilizador/dashboard")}
          ...
          <div className="bg-white rounded-xl p-6 border border-gray-200 transition-shadow" style={{ margin: '15px' }}>
            ...
            <div className="flex items-start justify-between mb-4">
              <div className="flex-1">
                <h1 className="text-2xl font-bold text-blue-600 mb-2 hover:underline transition-all">
                  {proposta.nome || proposta.titulo || 'Sem título'}
                </h1>
            ...
            <div className="flex justify-start gap-3">
              <button
                ...
                className={`flex items-center gap-2 border border-gray-300 rounded-full p-2 transition-all bg-white ${isFavorito ? 'text-blue-600 hover:bg-blue-100' : 'text-gray-400 hover:bg-yellow-100 hover:text-yellow-500'}`}
                title={isFavorito ? 'Remover dos favoritos' : 'Adicionar aos favoritos'}
                style={{ background: 'white', border: '1px solid #d1d5db', padding: '0.5rem' }}
              >
                {isFavorito ? <FaStar /> : <FaRegStar />}
                <span className={`text-xs font-medium ${isFavorito ? 'text-blue-600' : 'text-gray-700'}`}>Favorito</span>
              </button>
