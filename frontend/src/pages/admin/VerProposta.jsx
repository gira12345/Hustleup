...
export default function VerProposta() {
  const { id } = useParams();
          ...
          <div className="bg-white rounded-xl p-6 border border-gray-200 transition-shadow" style={{ margin: '15px' }}>
            ...
            <div className="flex items-start justify-between mb-4">
              <div className="flex-1">
                ...
                <p className="text-xs text-gray-500 mb-4">
                  {(() => {
                    const empresaNome = proposta.empresa?.nome || proposta.Empresa?.nome || proposta.empresaNome || 'Sem empresa';
            ...
            {proposta.empresa && (
              <div className="border-t border-gray-200 pt-4 mb-6">
                <h3 className="text-sm font-semibold text-gray-700 mb-3">Empresa:</h3>
                <div className="bg-gray-50 rounded-lg p-4">
                  <div className="grid md:grid-cols-2 gap-4">
                    <div>
                      <p className="text-sm font-medium text-gray-900">{proposta.empresa.nome}</p>
                      {proposta.empresa.departamento && (
                        <p className="text-xs text-gray-600 mt-1">Departamento: {proposta.empresa.departamento}</p>
                      )}
                    </div>
                    <div>
                      {proposta.empresa.localizacao && (
                        <p className="text-xs text-gray-600">Localização: {proposta.empresa.localizacao}</p>
                      )}
                      {proposta.empresa.contacto && (
                        <p className="text-xs text-gray-600 mt-1">Contacto: {proposta.empresa.contacto}</p>
                      )}
                      {proposta.empresa.descricao && (
                        <p className="text-xs text-gray-600 mt-1">{proposta.empresa.descricao}</p>
                      )}
                    </div>
                  </div>
                </div>
            ...
            <div className="flex flex-wrap gap-3">
              <button
                onClick={() => navigate(`/admin/editar-proposta/${proposta.id}`)}
