...
export default function Propostas() {
  ...
  useEffect(() => {
    const estados = [...new Set(propostas.map(p => p.estado).filter(Boolean))];
    setEstadosDisponiveis(estados.sort());
  }, [propostas]);
  ...
  const buscarPropostas = async () => {
    try {
      ...
      setError(`Erro ao carregar propostas: ${err.response?.status || 'Sem resposta'}`);
      setPropostas([]);
    } finally {
      setLoading(false);
    }
  ...
  const filtrarPropostas = () => {
    return propostas.filter((p) => {
      ...
      const empresaMatch = p.empresa?.nome ? p.empresa.nome.toLowerCase().includes(pesquisa.toLowerCase()) : false;
      const pesquisaMatch = semPesquisa || tituloMatch || empresaMatch;
          ...
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-100">
              <tr>
                ...
                <th className="px-4 py-2 text-left text-sm font-medium">Empresa</th>
              ...
              {!loading && filtrarPropostas().length === 0 ? (
                ...
                filtrarPropostas().map((p) => (
                  <tr key={p.id}>
                    ...
                    <td className="px-4 py-2">{p.empresa?.nome || "-"}</td>
                    ...
                    <td className="px-4 py-2 space-x-2">
                      <button
                        onClick={() => navigate(`/admin/ver-proposta/${p.id}`)}
                      ...
                      <button
                        onClick={() => navigate(`/admin/editar-proposta/${p.id}`)}
